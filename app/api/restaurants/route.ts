  ...
  try {
    ...
    const raw = await page.locator('a').evaluateAll((anchors) => {
      const isPlaceHref = (href: string) =>
        /(?:m\.)?place\.naver\.com\/(?:restaurant|place)\//i.test(href) ||
      ...
      return anchors
        ...
        .filter((x) => x.text.length > 1 && isPlaceHref(x.href));
    });
    ...
    for (const x of raw) {
      ...
      const name = clean(
        t.split(/별점|방문자 리뷰|블로그 리뷰|영업|메뉴|거리|현재 영업|리뷰/)[0]
      ).slice(0, 60);

      if (!name || blockedNames.has(name) || seen.has(name)) continue;
      seen.add(name);
      ...
      items.push({
        name,
        rating,
        reviews: visitor ? `방문자 리뷰 ${visitor}` : blog ? `블로그 리뷰 ${blog}` : undefined,
        distance,
        href: x.href,
      });
    ...
    if (!items.length) {
      return NextResponse.json({
        error: '네이버 지도에서 식당 상세 링크를 찾지 못했습니다. 네이버 지도 화면 구조가 변경되었을 수 있습니다.'
      }, { status: 502 });
    }
    ...
    console.error(e);
    return NextResponse.json({
      error: e instanceof Error ? `검색 실패: ${e.message}` : '검색 중 오류가 발생했습니다.'
    }, { status: 500 });
  } finally {
